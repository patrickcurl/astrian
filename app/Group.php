...
class Group extends Model
{
    ...
    public static function testRevisions()
    {
        $model = self::find(2);
        $revisions[] = array(
            ...
            'key' => self::CREATED_AT,
            'old_value' => null,
            'new_value' => $model->{self::CREATED_AT},
            'user_id' => $model->getSystemUserId(),
            'created_at' => new \DateTime(),
            'updated_at' => new \DateTime(),
        );
        $revision = Revisionable::newModel();
        DB::table($revision->getTable())->insert($revisions);
        Event::dispatch('revisionable.saved', array('model' => $model, 'revisions' => $revisions));
    }
}
